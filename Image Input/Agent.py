...
#class that represents knowledge-based agent to solve Raven's progressive matrices
class Agent:
    def __init__(self):
        pass
    ...
    def solveProblem(self,problem):
        ...
        for obj in C_obj:
            ...
            if("deleted" not in newValues ):
                    ...
                    added_attributes= list(set(newValues) - set(C_obj[obj].getAttributes().keys()))
                    if(added_attributes):
                    ...
                    for keyValue in CFrame[obj].items():
                        if(keyValue[0] in newValues):
                            ...
                            if(keyValue[0] == "shape"):
                                if(keyValue[1] == DFrame[obj].get("shape")):
    ...
    def changeShape(self, AFrame, BFrame, CFrame, delta, DFrame):
        for change in delta.keys():
            if(delta[change] != "deleted" and "shape" in delta[change].keys()):
